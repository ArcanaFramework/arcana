...
class FileFormat(object):
    ...
    def set_converter(self, file_format, task_interface, inputs=None,
        ...
        task_interface : pydra.engine.core.TaskBase
        ...
        **default_kwargs
        ...
        def converter(name, **kwargs):
            ...
            @mark.annotate(
                ...
                'return': {
                    'path': str}})
            def get_paths(file_group: FileGroup, **kwargs):
            ...
            conv_kwargs = {inputs[i]:
                           getattr(wf.get_paths.lzout, i) for i in inputs}
            conv_kwargs.update(default_kwargs)
            conv_kwargs.update(kwargs)
            wf.add(task_interface(name='converter', **conv_kwargs))

            # Collect the paths generated by the converter and create a
            # file-group object again
            collect_path_annotations = {
                'path': str,
                'return': {
                    'file_group': FileGroup}}
            ...
            # Convert into a task
            collect_paths = mark.task(collect_paths)
