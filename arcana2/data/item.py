    ...
    def __repr__(self):
        return ("{}('{}', {}, '{}', subj={}, vis={}, exists={}, "
                ...
                .format(
                    ...
                    self.timepoint_id, self.exists, self.quality,
...
class Field(DataItem, FieldMixin):
    ...
    def __repr__(self):
        return ("{}('{}',{} '{}', subject={}, timepoint={}, exists={})"
                .format(
                    type(self).__name__, self.path,
                    (" {},".format(self._value)
                     if self._value is not None else ''),
                    self.frequency, self.subject_id,
                    self.timepoint_id, self.exists))
...
class Record(object):
    ...
    a given session (or timepoint, subject, analysis summary). It also records the
    ...
    frequency : DataFrequency
        ...
        per 'session', 'subject', 'timepoint', 'group_timepoint', 'group' or 'dataset'
    subject_id : str | None
        The subject ID the record corresponds to. If None can be a per-timepoint or
        per-analysis summary
    timepoint_id : str | None
        The timepoint ID the record corresponds to. If None can be a per-subject or
        per-analysis summary
    namespace : str
    ...
    def load(cls, path, local_path, data_node):
        ...
        frequency : DataFrequency
            ...
            per 'session', 'subject', 'timepoint', 'group_timepoint', 'group' or
            'dataset'
        subject_id : str | None
            The subject ID of the provenance record
        timepoint_id : str | None
            The timepoint ID of the provenance record
        local_path : str
