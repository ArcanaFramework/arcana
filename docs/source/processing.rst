...
.. code-block:: python
    ...
    from arcana.data.formats import common, medimg

    dataset = Dataset.load('myuni-xnat//myproject:training')

    dataset.add_source('T1w', format=medimg.Dicom, path='.*mprage.*',
                       is_regex=True)
    dataset.add_source('T2w', format=medimg.Dicom, path='.*t2spc.*',
                       is_regex=True)
    ...
    dataset.apply_workflow(
        workflow=Freesurfer(
            ...
            param1=10.0,
            param2=20.0),
        inputs=[('T1w', 'in_file', medimg.NiftiGz),
                ('T2w', 'peel', medimg.NiftiGz)],
...
.. code-block:: python
    ...
    dataset.apply_workflow(
        workflow=FAST(
            name='segmentation',
            method='a-method'),
        sources=[('T1w', 'in_file', medimg.NiftiGz)],
        sinks=[('fast/gm', 'gm', medimg.NiftiGz)])
...
.. code-block:: console
    ...
    $ arcana apply workflow 'file///data/openneuro/ds00014:test' segmentation \
      ...
      --source T1w in_file medimg:NiftiGz \
      --sink fast/gm gm medimg:NiftiGz \
