...
explicitly use the ``arcana.mark.column`` function to include some basic
...
provided to the :func:`arcana.mark.analysis` class decorator.
...
derive data columns. The :func:`arcana.mark.pipeline`
...
The ``arcana.mark.output`` decorator is used to specify an output method
