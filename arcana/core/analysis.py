...
# Reserved names that can't be used for attributes in a class
RESERVED_ATTR_NAMES = ("dataset", "menu", "stack")
...
def make_analysis_class(cls, space: DataSpace):
    ...
    for name, attr in list(cls.__dict__.items()):
        if name in RESERVED_ATTR_NAMES:
            raise ArcanaDesignError(
                f"Cannot use reserved name '{name}' for attribute in '{cls.__name__}' "
                "analysis class"
            )
        ...
        try:
            ...
            if attr_type == "column":
                if cls.__annotations__[name] is not str:
                    attr.metadata["datatype"] = cls.__annotations__[name]
                    cls.__annotations__[name] = str  # FIXME: change to Column
    ...
    # parameters
    analysis_cls = attrs.define(
        type(cls.__name__, (Analysis, cls), {"__analysis__": None})
    )
