...
class Analysis:
    ...
    dataset: Dataset = attrs.field(default=None, validator=_dataset_validator)
    __spec__: AnalysisSpec = None  # type: ignore
    ...
    def __attrs_post_init__(self):
        ...
        for spec in self.__spec__.column_specs:
            ...
            if isinstance(column, str):
                ...
                setattr(self, spec.name, column)
            elif column not in (None, attrs.NOTHING) and not isinstance(
                column, DataColumn
            ):
                raise ValueError(
                    f"Value passed to '{spec.name}' ({column}) should either be a "
        ...
        for spec in self.__spec__.subanalysis_specs:
            ...
            if isinstance(subanalysis, dict):
                ...
                subanalysis = Subanalysis(
                    spec=spec,
                    analysis=spec.type(name=spec.name, dataset=self.dataset, **kwargs),
                    parent=self,
                )
                setattr(self, spec.name, subanalysis)
            elif not isinstance(subanalysis, Subanalysis):
    ...
    def make(cls, klass: type, space: ty.Type[DataSpace]) -> ty.Type["Analysis"]:
        ...
        if Analysis not in klass.__mro__:
            raise ArcanaDesignError(
                "Classes decorated by the @analysis decorator must inherit from the "
                "Analysis base class"
            )
        ...
        for name, attr in list(klass.__dict__.items()):
            ...
            if isinstance(attr, (_Inherited, _MappedFrom)):
                ...
                if dtype is None:
                    # Copy type annotation across to new class if it isn't present
                    dtype = resolved_dtype
                elif dtype is not resolved_dtype and not issubclass(
                    dtype, resolved_dtype
                ):
                    raise ArcanaDesignError(
                        f"Cannot change datatype of {name} from {resolved_dtype} to "
        ...
        for spec in subanalysis_specs:
            ...
            for col_or_param in itertools.chain(column_specs, parameters):
                if (
                    ...
                    and col_or_param.mapped_from[0] == spec.name
                ):
                    resolved_mappings.append(
                        (col_or_param.mapped_from[1], col_or_param.name)
                    )
        ...
        for base in klass.__mro__[1:]:
            try:
                base_spec: AnalysisSpec = base.__spec__  # type: ignore
            except AttributeError:
                continue
            else:
                if base_spec is None:
                    continue
            base = ty.cast(ty.Type[Analysis], base)
            if base_spec.space is not space:
                missing_freqs = [f for f in base_spec.space if f not in space]
                if missing_freqs:
                    raise ValueError(
                        "Cannot redefine the space that an analysis operates on from "
                        f"{base_spec.space} to {space} as it doesn't define values for "
                        f"{missing_freqs}"
                    )
            # Append column specs, parameters and subanalyses that were inherited from
            # base classes
            for lst, base_lst in (
                (column_specs, base_spec.column_specs),
                (parameters, base_spec.parameters),
                (subanalysis_specs, base_spec.subanalysis_specs),
            ):
                if not_inherited_explicitly := [
                    x.name
                    for x in lst
            ...
            for lst, base_lst in (
                ...
                for base_method in base_lst:
                    ...
                    # Check pipeline builders to see that they don't remove outputs
                    if isinstance(method, PipelineConstructor):
                        if isinstance(base_method, PipelineConstructor):
                            base_method = ty.cast(PipelineConstructor, base_method)
                        else:
                            raise ArcanaDesignError(
                                f"Cannot override {base_method} with pipeline constructor "
                                f"in {klass}"
                            )
                        if missing_outputs := [
                            o for o in base_method.outputs if o not in method.outputs
        ...
        for attr in to_convert_to_attrs:
            attrs_field = attr.to_attrs_field()
            setattr(klass, attr.name, attrs_field)
            klass.__annotations__[attr.name] = attr.type

        # Add the analysis spec to the __spec__ attribute
        klass.__spec__ = analysis_spec  # type: ignore
        klass.__annotations__["__spec__"] = AnalysisSpec

        # Create class using attrs package, will create attributes for all columns and
        # parameters
        attrs_klass = attrs.define(kw_only=True, auto_attribs=False)(klass)

        # Set the class that newly attribute attributes were defined in
        for attr in to_set_defined_in:
            # Register the attribute/method as being defined in this class if it has
            # been either added or created
            if isinstance(attr, BaseMethod) or len(attr.modified) == len(
                attr.defined_in
            ):
                object.__setattr__(attr, "defined_in", attr.defined_in + (attrs_klass,))
