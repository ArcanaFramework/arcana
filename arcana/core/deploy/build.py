...
def install_python(packages: ty.Iterable[PipSpec], build_dir: Path,
    ...
    try:
        arcana_pkg_spec = packages[PACKAGE_NAME]
    except KeyError:
        packages[PACKAGE_NAME] = PipSpec(
            version=__version__,
            extras=list(arcana_install_extras))
    else:
        arcana_pkg_spec.extras.extend(arcana_install_extras)
