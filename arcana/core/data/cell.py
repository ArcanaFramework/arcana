...
from arcana.core.analysis.salience import DataQuality  # @ignore reshadowedImports
...
class DataCell:
    """
    An location within a data frame at which an item of data can be located. Contains
    all the metadata associated with a data item, including where in the dataset
    it can be found, and its quality and provenance. The actual data can be accessed
    via the ``item`` property, which will pull the data to the local host if required.

    Parameters
    ----------
    column : DataColumn
        The data row within a dataset that the cell belongs to
    row : DataRow
        The data row within a dataset that the cell belongs to
    path : str
    ...
    order : int | None
        The order in which the cell appears in the row it belongs to
    ...
    empty : bool
        Whether or not the cell contains a data item
    ...
    empty: bool = attrs.field(default=True)
    ...
    _item: ty.Any = attrs.field(default=None)

    @row.validator
    def row_validator(self, _, row):
        if row.dataset is not self.column.dataset:
            raise ValueError(
                f"Dataset doesn't match between provided row ({row}) and column "
                f"({self.column})"
            )
    ...
    def item(self):
        if self.empty:
            if self.column.is_sink:
                raise ArcanaDataNotDerivedYetError(
                    f"Cannot access item of {self}, as it is empty"
                )
        if self._item is None:
            self._item = self.store.get(self)
        return self._item
