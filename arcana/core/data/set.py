...
class Dataset:
    ...
    _root_node: DataNode = attr.ib(default=None, init=False, repr=False, eq=False)
    ...
    def load(
        ...
        **kwargs
        ...
        Dataset
            the loaded dataset"""
        if store is None:
            # TODO rename id to avoid confusion
    ...
    def columns_validator(self, _, columns):
        wrong_freq = [
            m for m in columns.values() if not isinstance(m.frequency, self.space)
        ]
        if wrong_freq:
            raise ArcanaUsageError(
    ...
    def add_source(
        self, name, format, path=None, frequency=None, overwrite=False, **kwargs
    ):
    ...
    def node(self, frequency=None, id=None, **id_kwargs):
        ...
        **id_kwargs : Dict[str, str]
        ...
        if not frequency:
            if id is not None:
                raise ArcanaUsageError(f"Root nodes don't have any IDs ({id})")
    ...
    def nodes(self, frequency=None, ids=None):
        ...
        if frequency is None:
            return chain(*(d.values() for d in self.root_node.children.values()))
        frequency = self._parse_freq(frequency)
    ...
    def add_node(self, ids, frequency):
        ...
        """
        logger.debug(f"Adding new %s node to %s dataset: %s", frequency, self.id, ids)
        frequency = self._parse_freq(frequency)
        ...
        if node.id in node_dict:
            raise ArcanaDataTreeConstructionError(
                f"ID clash ({node.id}) between nodes inserted into data " "tree"
            )
    ...
    def apply_pipeline(
        self, name, workflow, inputs, outputs, frequency=None, overwrite=False
    ):
